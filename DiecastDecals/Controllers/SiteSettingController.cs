...
namespace DiecastDecals.Controllers
{
    public class SiteSettingController : Controller
    {
        ...
        private String ProcessHeaderLogo(ref String Doc_Name, HttpPostedFileBase file)
        {
            if (file == null)
            {
                ViewBag.HEADER_LOGO = Request["hf_HEADER_LOGO"];
        ...
        private String ProcessFooterLogo(ref String Doc_Name, HttpPostedFileBase file)
        {
            if (file == null)
            {
                ViewBag.FOOTER_LOGO = Request["hf_FOOTER_LOGO"];
                //ViewBag.LOGO_NAME = Request[ViewBag.hf_LOGO_NAME];
                return String.Empty;
            }
            else
        ...
        [HttpPost]
        public ActionResult btn_Head_Save_Click(FormCollection form, HttpPostedFileBase fu_Logo, HttpPostedFileBase fu_footer)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (String.IsNullOrEmpty(errMsg))
                    {
                        errMsg = ProcessFooterLogo(ref FOOTER_LOGO, fu_footer);
                        if (!String.IsNullOrEmpty(errMsg))
                        {
                            TempData["JavaScriptMsg"] = string.Format("OpenTab1('" + Message.msgErrCommon + "');");
                            // MessageBox(3, errMsg, "");
                    ...
                    oBMAST.FOOTER_LOGO = ViewBag.FOOTER_LOGO;
